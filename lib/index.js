...
axios.get(url)
    .then(async (response) => {
        ...
        const chainlinkData = []
        for (let i = 1; ; i++) {
            const aggregatorAddress = $(`#content-container > section.content-body.grid-75 > div.markdown-body > div:nth-child(3) > div > table tr:nth-child(${i}) td:nth-child(2)`).text().trim()
            const pair = $(`#content-container > section.content-body.grid-75 > div.markdown-body > div:nth-child(3) > div > table tr:nth-child(${i}) td:nth-child(1)`).text()
